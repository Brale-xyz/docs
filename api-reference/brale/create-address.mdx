...
Returned when the submitted `account_number` and `routing_number` combination matches bank details that Brale has previously disabled (for example, after certain terminal ACH returns). See [Address lifecycle → Disabled external bank addresses](/key-concepts/addresses#disabled-external-bank-addresses) for the full behavior.
...
- Re-registering the same bank details does not create a usable replacement address or restore the existing address.
- Retrying with a different idempotency key does not bypass the restriction.
- Always submit accurate bank information. Do not modify bank details merely to work around the restriction.
- If the bank information has genuinely changed, register the corrected bank details.
- If the existing details are believed to remain valid, [contact Brale support](mailto:support@brale.xyz) for review. Reactivation of the underlying bank account is at Brale's discretion and is not guaranteed.
