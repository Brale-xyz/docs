...
| `worldchain_testnet` | Worldchain Sepolia Testnet |
| `xion_testnet` | Xion Testnet |

## Supported `transfer_type` values by `value_type`

Not every `value_type` is available on every `transfer_type`. The tables below list the supported `transfer_type` values for USDC, USDT, RLUSD, PYUSD, and USDG.
...
### Supported `transfer_type` values for USDG
