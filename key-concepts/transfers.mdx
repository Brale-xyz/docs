...
<p class="intro-subheading">
  Transfers enable the movement of funds between fiat and stablecoins,
  stablecoin-to-stablecoin conversions, and stablecoin-to-fiat transactions.
</p>
...
- `transfer_type`: The payment rail used (Wire, ACH, Polygon, Solana, SPEI, etc.)
  - Note: In API requests, use lowercase ids for transfer_type from Coverage (e.g., wire, solana)
...
| Flow                                | Source                 | Destination                         | Description                                | Party     |
| ----------------------------------- | ---------------------- | ----------------------------------- | ------------------------------------------ | --------- |
| Onramp<br />(Fiat → Stablecoin)     | USD / wire / ach_debit | SBC / base                          | Mint SBC on Base funded by wire deposit    | 2nd Party |
| Offramp<br />(Stablecoin → Fiat)    | SBC / solana           | USD / ach_credit                    | Redeem SBC on Solana to USD via ACH        | 2nd Party |
| Swap<br />(Stablecoin ↔ Stablecoin) | USDC / solana          | SBC / solana                        | 1:1 swap from USDC to SBC with no slippage | 2nd Party |
| On-chain Payout                     | USDC / polygon         | USDC / polygon (external address)   | Pay a recipient wallet                     | 3rd Party |
| USDC off-ramp (wire)                | USDC / polygon         | USD / wire                          | Offramp USDC to wire                       | 2nd Party |
| SBC branded ACH payout (ACH only)   | SBC / base             | USD / ach_credit + brand (ACH only) | ACH payout with brand on statement         | 3rd Party |
...
Swap USDC to your own stablecoin (SBC). All stablecoin swaps are 1:1 with no slippage.
