...
If the account is a managed customer account, note that [transactional accounts](/guides/transactional-accounts) cannot originate on-chain transfers. See [Managed Accounts](/guides/managed-accounts) to compare account models.
...
- Production integrations can subscribe to [`transfer.completed`](/webhooks/webhook-events#transfer-completed) and [`transfer.failed`](/webhooks/webhook-events#transfer-failed) [webhooks](/webhooks/overview) to receive signed events when a transfer reaches `complete` or `failed`. Use polling or retrieval endpoints for development, fallback handling, and reconciliation.
...
- Standard and Same-Day ACH transfers are generally limited to $50,000 per transaction, including deposits and payouts.
- RTP transfers are generally limited to $50,000 per transaction.
- Wire transfers do not have a standard hard maximum. Wires above $3,000,000 may require additional review and processing time.
- Stablecoin transfers do not have a standard per-transaction maximum but may be subject to account-specific controls, compliance review, available liquidity, and blockchain network constraints.

Limits may vary by account, payment rail, transaction direction, jurisdiction, and risk profile. Contact Brale before initiating a larger transfer or if you need account-specific limits.
