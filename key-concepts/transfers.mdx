...
title: "Transfers"
seo:
  title: "Transfers — onramp, offramp, swap, and payout"
  keywords: "transfer, onramp, on-ramp, offramp, off-ramp, swap, payout, wire, ACH, fiat to stablecoin, stablecoin to fiat"
...
| `page[next]` | Cursor from `pagination.next` | — |
...
Use the `pagination.next` cursor from the previous response to page forward:

```bash
curl "https://api.brale.xyz/accounts/${ACCOUNT_ID}/transfers?page[next]=g3QAAAABdw..." \
  -H "Authorization: Bearer ${AUTH_TOKEN}"
```
