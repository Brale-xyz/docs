...
KYB verification can require supporting documents (for example, proof of incorporation or beneficial-owner identification). Documents are submitted in two steps:

1. **Stage the documents.** Upload each KYB document to obtain a `doc_submission_id`. Staged submissions are not yet associated with an account.
2. **Link the submissions to the account.** Call [Link Verification Documents](/api-reference/brale/link-verification-documents) (`PATCH /accounts/{account_id}`) with the staged `doc_submission_ids` to attach them to the account for review.

The account `status` remains `pending` while Brale reviews the submitted documents, then transitions to `complete` if KYB passes or `rejected` if declined.
