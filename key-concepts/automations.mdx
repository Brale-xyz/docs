...
<Warning>
Automations on a transactional account (`account_type: transactional`) may only target external destinations. A Brale-custodied destination is rejected. See [Account types](/key-concepts/account-types) and [Individual accounts](/guides/individual-accounts).
</Warning>
