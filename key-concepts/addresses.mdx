...
<Warning>
Internal (custodial) addresses are only available on business, custodial accounts. Individual, transactional accounts (`account_type: transactional`) do not get an automatic custodial wallet at creation and cannot hold internal (`type=internal`) addresses. Their external addresses are destination-only — they can receive value on-chain but cannot be the source of a transfer. See [Account types](/key-concepts/account-types) and [Individual accounts](/guides/individual-accounts).
</Warning>
