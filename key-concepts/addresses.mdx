...
| Status     | Meaning                                                                                                                              | Set by         | Customer restoration                                                                                     |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------ | -------------- | -------------------------------------------------------------------------------------------------------- |
| `pending`  | The address is being provisioned and is not ready for transfers.                                                                     | Brale          | Normally no action is required.                                                                          |
| `active`   | The address is available for its supported transfer types.                                                                           | Brale          | —                                                                                                        |
| `archived` | The customer intentionally removed the external address from use while preserving its history.                                       | Customer / API | Restore with `PATCH /accounts/{account_id}/addresses/{address_id}` and `{ "status": "active" }`.         |
| `disabled` | Brale restricted the underlying bank details, such as after certain terminal ACH returns.                                            | Brale          | Cannot be restored through the API; [contact Brale support](mailto:support@brale.xyz) for review.        |

<Warning>
`archived` and `disabled` are different states. An archived external address can be restored through the API. A disabled external bank address cannot be reactivated through the API or bypassed by registering the same account-number and routing-number combination again.
</Warning>

### Archive and unarchive (external addresses only)

You can change the lifecycle state of an **external** address between `active` and `archived` by sending a `status` update to the [PATCH address endpoint](/api-reference/brale/update-address):
...
- **Unarchive:** `PATCH /accounts/{account_id}/addresses/{address_id}` with `{ "status": "active" }` restores an archived address.
...
`PATCH /accounts/{account_id}/addresses/{address_id}` with `{ "status": "active" }` only restores an external address that is `archived`. It does not restore a Brale-disabled address.

See the [PATCH address endpoint reference](/api-reference/brale/update-address) for request and response examples.

### Disabled external bank addresses

`disabled` is a Brale-enforced state for an external bank address, applied to the underlying bank details rather than set by the customer. Brale may apply `disabled` after certain terminal ACH returns. `R03` ("No Account / Unable to Locate Account") is a confirmed example; not every ACH failure disables a bank address, and Brale does not publish an exhaustive list of disabling return codes.

Current behavior:

- A disabled external bank address cannot be used for new transfers.
- Customers cannot change a disabled address back to `active` through the API.
- Attempting to re-register the same account-number and routing-number combination through [`POST /accounts/{account_id}/addresses/external`](/api-reference/brale/create-address) returns `422 external_address_disabled`. Retrying with a different idempotency key does not bypass the restriction.
- An ACH-triggered disablement may also prevent the underlying bank details from being used for other rails, including wire.
- Multiple `Address` records may reference the same underlying bank details. The observable restriction is on reuse of those bank details; Brale does not guarantee that every duplicate `Address` row is updated together.

If the bank information has genuinely changed, register the corrected bank details as a new external address. If the existing details are believed to remain valid, [contact Brale support](mailto:support@brale.xyz) to review the restriction. Brale support can manually reactivate the underlying bank account when appropriate; reactivation is not guaranteed.
...
When your customer's Account is onboarded, Brale automatically generates internal custodial addresses on all supported blockchains, so the default wallets are ready to use without any manual setup. You can also provision **additional** custodial wallets on demand via [`POST /accounts/{account_id}/addresses/internal`](/api-reference/brale/create-internal-address). If an address is visible under an account-scoped endpoint but not globally, use the account-scoped endpoint.

<Warning>
Internal (custodial) addresses are only available on custodial accounts, business or individual. Transactional accounts (`account_type: transactional`) do not get an automatic custodial wallet at creation and cannot hold internal (`type=internal`) addresses. Their external addresses are destination-only — they can receive value on-chain but cannot be the source of a transfer. See [Account types](/key-concepts/account-types) and [Individual accounts](/guides/individual-accounts).
</Warning>

Holding multiple internal wallets within a single account lets you segment funds — for example per end-user, per-purpose (operating vs. escrow vs. reserve), or per-business-unit — distinguished by `name`. This replaces the prior workaround of creating separate accounts or organizations just to keep balances apart.
...
For non-deterministic chains (Spark, Canton, Hedera), the address returns with `status: "pending"` and `address: null`, then transitions following the [Address lifecycle](#address-lifecycle) table above (`pending` → `active`).
...
Use the [Create a new external address](/api-reference/brale/create-address) endpoint to register a bank account when you already have the bank details. This path supports `wire`, `ach_credit`, `same_day_ach_credit`, and `rtp_credit`. It does **not** support `ach_debit`.

**POST** `/accounts/{account_id}/addresses/external`

<Note>
For Direct bank entry, `bank_address` and `beneficiary_address` are required only when `transfer_types` includes `wire`. They are optional for ACH/RTP-only addresses using `ach_credit`, `same_day_ach_credit`, or `rtp_credit`. Omit these fields for ACH/RTP-only addresses unless you have accurate values—do not use placeholder or generic bank addresses.
</Note>

| Field                 | ACH Credit | Same Day ACH Credit | RTP Credit (`rtp_credit`) | Wire     |
| --------------------- | ---------- | ------------------- | ------------------------- | -------- |
...
To check whether RTP eligibility has been confirmed, [retrieve the Address](/api-reference/brale/get-address) with `GET /accounts/{account_id}/addresses/{address_id}` and inspect `transfer_types` for `rtp_credit`.
...
`additional_transfer_types` is a list of transfer types to **add** to this Address. These are **merged** into the address's existing `transfer_types` — you do not need to resend the full existing list.

- The values in `additional_transfer_types` are **added** to the Address's existing `transfer_types`.
- Existing `transfer_types` are preserved even if you don't include them in the request. Only send the rails you want to add.
