...
- Cause: `POST /accounts` was called with a combination of `entity_type`, `account_type`, and `verification_mode` outside the supported rows. Supported combinations: `business` + `custodial` + `standard`, `business` + `transactional` + `standard`, `individual` + `transactional` + `standard`, and `individual` + `transactional` + `reliance`. Individual custodial is not currently supported.
