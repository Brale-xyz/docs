...
<Info>
  Where available, use [`transfer.completed`](/webhooks/webhook-events#transfer-completed) and [`transfer.failed`](/webhooks/webhook-events#transfer-failed) [webhooks](/webhooks/overview) for real-time production notifications when a transfer reaches a terminal state. Polling remains useful for fallback handling, reconciliation, and retrieving the latest transfer state.
</Info>
