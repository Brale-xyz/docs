...
<Info>
  Where available, use [`transfer.completed`](/webhooks/webhook-events) [webhooks](/webhooks/overview) for real-time production completion notifications. Polling remains useful for fallback handling, reconciliation, and retrieving the latest transfer state.
</Info>
