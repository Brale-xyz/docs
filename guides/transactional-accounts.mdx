...
4. **Deliver value to the destination.**\
   Use an eligible source account or a supported program flow to send value to the external address. For example, an on-ramp [automation](/key-concepts/automations) or a [transfer](/key-concepts/transfers) owned by your managing account can target it.
