...
For transfers to parties also custodied by Brale, this acceptance is performed automatically.
...
```shell Get current ledger end offset
curl -s GET \
...
```shell Get the active contracts as of the offset
curl -s \
...
```shell Use the Token Standard API to get the contextual information
curl -s \
