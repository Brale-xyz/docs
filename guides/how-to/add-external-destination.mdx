...
Direct bank entry supports `wire`, `ach_credit`, `same_day_ach_credit`, and `rtp_credit`. `bank_address` and `beneficiary_address` are required only when `transfer_types` includes `wire`. Omit those fields for ACH/RTP-only addresses unless you have accurate values—do not enter placeholder or generic bank addresses.

<Note>
Need `ach_debit`? Use the [Plaid-linked bank account flow](/key-concepts/addresses#create-via-plaid-linked-bank-account). Direct bank entry supports `wire`, `ach_credit`, `same_day_ach_credit`, and `rtp_credit`, but **not** `ach_debit`.
</Note>

**ACH/RTP Direct bank entry**
