...
description: "Issue and operate your own tokens using Brale for key custody, mint and burn execution, and wallet orchestration."
---
...
4. **Mint / Burn / Transfers**
   ...
   - Use `POST /accounts/{account_id}/tokens/mints` to **mint** tokens into a wallet.
   - Use `POST /accounts/{account_id}/tokens/burns` to **burn** tokens from a wallet.
   - Use `POST /accounts/{account_id}/tokens/transfers` to move tokens between custodial and external addresses.

5. **Reporting**

   - List and retrieve prior operations with the tokenization list/get endpoints (mints, burns, transfers).
...
**POST** `https://api.brale.xyz/accounts/{account_id}/tokens/transfers`
