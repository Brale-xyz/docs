...
The workflow below shows the transactional flow. For a custodial account, set `account_type: custodial` in step 1; on approval the account receives Brale-custodied wallets automatically, and steps 3–5 do not apply — work with its internal addresses instead, as described in [Custodial Accounts](/guides/custodial-accounts).
...
See [Branded Automations](/key-concepts/automations#branded-automations) for the full behavior and business-account example.
