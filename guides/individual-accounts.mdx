...
1. **Create the account.**\
   ...
   Non-US individuals are supported. `individual.address.state` is free-form and the postal code accepts any format; send the identity document type that matches the individual's country. See [Identity types](/key-concepts/accounts#identity-types) for the accepted values and [Creating a non-US individual account](/key-concepts/accounts#creating-a-non-us-individual-account) for an example.
