...
<Update label="2026-05-08" description="Week of May 4" tags={["New"]}>
  ## Main USD (USDM) now available

  **USDM** (Main USD) is available as a new value type, issued by Brale. Use `USDM` as a `value_type` in the API to mint, burn, and transfer Main USD alongside other Brale-issued stablecoins. See [value types](/coverage/value-types) for the full registry.
</Update>
