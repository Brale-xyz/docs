...
<Steps>
  ...
  <Step title="Track transfer status">
    Transfers may complete asynchronously. For real-time production updates, subscribe to [`transfer.completed`](/webhooks/webhook-events#transfer-completed) and [`transfer.failed`](/webhooks/webhook-events#transfer-failed) [webhooks](/webhooks/overview) so Brale can send your application a signed event when the transfer reaches `complete` or `failed`.
